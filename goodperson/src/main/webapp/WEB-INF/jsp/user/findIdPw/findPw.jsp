...
function chkData(){
	...
	var name	= $("#name");
	var email 	= $("#email");
	var regId 	  	= /[ㄱ-ㅎㅏ-ㅣ가-힣`.,\[\]\{\}\(\)\-_&=+~!@#$%^*|\\\'\";:\/?]/g;
	var regNm	  	= /[ㄱ-ㅎㅏ-ㅣ`.,\[\]\{\}\(\)\-_&=+~!@#$%^*|\\\'\";:\/?]/g;
	var regEm  	= /[ㄱ-ㅎㅏ-ㅣ가-힣A-Z`,\[\]\{\}\(\)\-_&=+~!#$%^*|\\\'\";:\/?]/g;
...
$(function(){
	$('#btn_submit_pw').on("click", function(){
		var flag = chkData();
		
		if(!flag){
			return false;
		}//end if
		$("#find_f").attr("action", "<c:url value='/user/searchPw.do'/>");
...
<div class="findIdPw_wrap">
	<div class="findIdPw_header_wrapper">
		<header class="findIdPw_header">
			<span class="findIdPw_notice"><h3>비밀번호 찾기</h3></span>
			<span class="findIdPw_header_otherlink">
				<a href="<c:url value='/user/findIdPw/findId.do'/>">아이디 찾기</a>
			</span>
		</header>
	</div>
				...
				<c:when test="${fn:length(map) < 2}">
					...
					<div class="findIdPw_inputField">
						<div>
							<form id="find_f" name="find_f">
								<div class="findIdPw_inputId"><input type="text" id="id" name="id" maxlength="20" placeholder="아이디"/></div>
					...
					<div class="findIdPw_btnField_wrapper">
						<div class="findIdPw_btnField">
							<a class="btn_comm" id="btn_submit_pw">찾기</a>
