...
$(function() {
	...
	$('#id').keyup(function(evt){
		if(!(evt.keyCode>36 && evt.keyCode<41)){
			...
			var obj_id		= $(this).val();
			$(this).val(obj_id.replace(/[^a-z0-9_]/gi,""));
		}
	});
	...
	$('#name').keyup(function(evt){
		var obj_name 	= $(this);
		var tmpreg 		= /[a-z0-9]|[ \[\]{}()<>?|`~!@#$%^&*-_+=,.;:\"'\\]/g;
		obj_name.val(obj_name.val().replace(tmpreg,""));
	});
	$('#mail1, #mail2').keyup(function(evt){
		var obj_m1 = $(this);
		obj_m1.val(obj_m1.val().replace(/[^a-z0-9]$/gi,""));
	});
	...
	$('#detailAddr').keyup(function(evt){
		var obj_deAddr 	= $(this);
		obj_deAddr.val(obj_deAddr.val().replace(/[^ㄱ-ㅎㅏ-ㅣ가-힣0-9-\s,]/g, "")); // \s : 공백허용.
	});
...
<div id="wrap" style="height:930px;">
	<div id ="logo">
		<img src="<c:url value='/common/images/logo/logo.png'/>" alt="/common/images/logo/logo.png" onclick="location.href='<c:url value="/main.do"/>' "/>
	</div>
	<main id="content">
		<form id="join_form" method="post" action="<c:url value='/join/joinSuccess.do'/>">
			...
			<div class="row_group">
				<div id="idDiv" class="join_row">
					<span id="part_id">
						...
						<input type="text" id="id" name="id" maxlength="20" placeholder="아이디" onblur="chkId()" style="width:360px;" />
					</span>
			...
			<div class="row_group">
				<div id="nameDiv" class="join_row">
					<span id="part_name">
						<input type="text" id="name" name="name" maxlength="6" placeholder="이름"/>
					</span>
					<div class="notice_area"><font color="red">*필수</font> 이름은 한글 2~6자로 형식으로 입력하십시오.</div>
				</div>
				<div id="genderDiv" class="join_row join_gender">
					<span class="sex">
						<span class="gender" style="border-right: solid 1px #dcdcdc;">
							<input type="radio" id="male" name="sex" value="M" onclick="chkSex()" style="width:266px;">
							<label id="maleLb" for="male">남자</label>
						</span>
						<span class="gender">
							<input type="radio" id="female" name="sex" value="F" onclick="chkSex()" style="width:266px;">
				...
				<div id="birthDiv" class="join_row"  style="padding: 0px; display: table;">
					...
					<div id="part_yy">
						<input type="text" id="datepicker" name="yy" readonly="readonly" placeholder="생년월일" style="width:135px; height:25px; margin-right:4px;"/>
